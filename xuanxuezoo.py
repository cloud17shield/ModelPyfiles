...
label_rdd = sc.parallelize(label_rdd.take(1200), 12)
image_rdd = sc.parallelize(image_data.get_image().take(1200), 12)
print("label rdd count", label_rdd.count())
print("image_rdd count", image_rdd.count())
samples = image_rdd.zip(label_rdd).map(
...
lrModel.summary()
lrModel.save('hdfs:///lr/pet_adoption_speed.h5')
print('save success!')
